...
import time
import random
import math # <-- NOUVEL IMPORT POUR LA PAGINATION
...
# --- MODIFICATION PAGINATION : Nouvelles variables de session ---
if 'current_page' not in st.session_state:
    st.session_state.current_page = 1
if 'total_pages' not in st.session_state:
    st.session_state.total_pages = 0

# --- Fonction pour charger le CSS (VERSION ULTRA-FUTURISTE AVEC PAGINATION) ---
    ...
    if theme_name == "Cyberpunk":
        ...
        <style>
            ...
            body::before {
                ...
                background-size: 400% 400%; animation: gradientShift 15s ease infinite; z-index: -2;
            }
            ...
            .css-1d391kg, .css-17eq0hr {
                ...
                border-right: 1px solid rgba(0, 255, 255, 0.3); box-shadow: 0 8px 32px 0 rgba(31, 38, 135, 0.37);
            }

            /* --- WIDGETS --- */
            .stTextInput > div > div > input, .stSelectbox > div > div > select {
                background: rgba(255, 255, 255, 0.08); color: #00ffff; border: 1px solid rgba(0, 255, 255, 0.5);
            ...
            .stButton > button {
                ...
                border: 1px solid #00ffff; color: #00ffff; font-weight: bold; font-family: 'Orbitron', sans-serif;
                position: relative; z-index: 1; transition: all 0.3s ease; overflow: hidden;
            }
            .stButton > button::before {
                content: ''; position: absolute; top: 0; left: -100%; width: 100%; height: 100%;
                background: linear-gradient(90deg, transparent, rgba(0, 255, 255, 0.4), transparent); transition: left 0.5s;
            }
            .stButton > button:hover::before { left: 100%; }
            .stButton > button:hover {
                background: rgba(0, 255, 255, 0.2); box-shadow: 0 0 20px #00ffff, inset 0 0 10px rgba(0, 255, 255, 0.5);
                transform: translateY(-2px);
            }
            /* Style pour les boutons désactivés */
            .stButton > button:disabled {
                background: rgba(50, 50, 50, 0.2); border-color: #555; color: #666; cursor: not-allowed;
                box-shadow: none; transform: none;
            }

            /* --- TITRE GLITCH AVANCÉ --- */
            .glitch {
                font-size: 4.5rem; font-weight: 900; text-transform: uppercase; position: relative;
                color: #00ffff; font-family: 'Orbitron', sans-serif; letter-spacing: 5px; text-shadow: 0 0 10px #00ffff;
            }
            ...
            /* --- CARTES (GLASSMORPHISM ET EFFETS) --- */
            .metadata-card, .search-result-card {
                background: rgba(255, 255, 255, 0.07); backdrop-filter: blur(8px); border: 1px solid rgba(0, 255, 255, 0.3);
                border-radius: 15px; padding: 20px; transition: all 0.4s cubic-bezier(0.075, 0.82, 0.165, 1); position: relative; overflow: hidden;
            }
            .search-result-card { padding: 15px; }
            .metadata-card::before, .search-result-card::before {
                content: ''; position: absolute; top: -2px; left: -2px; right: -2px; bottom: -2px;
                background: linear-gradient(45deg, #00ffff, #ff00ff, #00ff00, #00ffff); border-radius: 15px; z-index: -1;
                opacity: 0; transition: opacity 0.4s ease; background-size: 400% 400%; animation: gradientShift 10s ease infinite;
            }
            .metadata-card:hover::before, .search-result-card:hover::before { opacity: 0.7; }
            .metadata-card:hover, .search-result-card:hover {
                transform: translateY(-5px) scale(1.02); box-shadow: 0 10px 40px rgba(0, 255, 255, 0.3); background: rgba(255, 255, 255, 0.1);
            }
            ...
            .pagination-info {
                color: #00ffff; font-family: 'Orbitron', sans-serif; font-size: 1.1em;
            }
...
def search_youtube(query, limit=30): # --- MODIFICATION : On cherche plus de résultats d'un coup
    videos_search = VideosSearch(query, limit=limit)
    return videos_search.result()['result']

# --- FONCTION DE TÉLÉCHARGEMENT AVEC YT-DLP (inchangée) ---
...
# --- NOUVELLE FONCTION POUR AFFICHER LA PAGINATION ---
...
if st.sidebar.button("Lancer la recherche") and search_query:
    with st.spinner("Recherche en cours..."):
        # --- MODIFICATION PAGINATION : On cherche 30 résultats et on calcule le nombre de pages ---
        results = search_youtube(search_query, limit=30)
        st.session_state.search_results = results
        st.session_state.total_pages = math.ceil(len(results) / 3)
        st.session_state.current_page = 1 # Toujours revenir à la page 1 après une recherche
    st.session_state.selected_video_url = None
    st.session_state.selected_video_data = None
    st.rerun()
...
# --- MODIFICATION PAGINATION : Affichage des résultats de la page actuelle ---
if st.session_state.search_results:
    st.subheader("Résultats de la recherche")
    
    # Calcul des indices pour la "tranche" de résultats à afficher
    results_per_page = 3
    start_index = (st.session_state.current_page - 1) * results_per_page
    end_index = start_index + results_per_page
    
    # On affiche uniquement les résultats de la page actuelle
...
# --- Affichage de la vidéo sélectionnée (inchangé) ---
