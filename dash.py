...
# --- Fonction pour charger le CSS (VERSION ULTRA-FUTURISTE) ---
    ...
    if theme_name == "Cyberpunk":
        ...
        <style>
            ...
            .glitch {
                ...
                color: #00ffff; font-family: 'Orbitron', sans-serif; letter-spacing: 5px;
            ...
            .metadata-card::before, .search-result-card::before {
                ...
                border-radius: 15px; z-index: -1; opacity: 0; transition: opacity 0.4s ease;
            ...
            .metadata-card img, .search-result-card img {
                border-radius: 10px; border: 1px solid rgba(0, 255, 255, 0.5);
            }
            ...
            .custom-spinner div {
                width: 15px; height: 15px; margin: 0 5px; background: #00ffff;
...
def download_media(url, format_choice):
    try:
        ...
        if format_choice == "MP4 (Vidéo)":
            ...
            # Construire la commande yt-dlp pour la meilleure qualité MP4
            command = [
                'yt-dlp',
                '-f', 'bv[ext=mp4]+ba[ext=m4a]/b[ext=mp4]', # Meilleure vidéo+audio ou meilleure vidéo seule
                '-o', output_path,
                url
            ]
            subprocess.run(command, check=True, capture_output=True, text=True)

            # Trouver le fichier téléchargé (yt-dlp ajoute l'extension)
            ...
            if downloaded_file:
                return downloaded_file, os.path.basename(downloaded_file), "video/mp4"
            else:
                raise Exception("Le fichier vidéo n'a pas pu être trouvé après le téléchargement.")

        elif format_choice == "MP3 (Audio)":
            # Définir le nom du fichier de sortie
            output_path = os.path.join(temp_dir, "audio.%(ext)s")
            # Construire la commande yt-dlp pour extraire et convertir en MP3
            command = [
                'yt-dlp',
                '-x', # Extraire l'audio
            ...
            if downloaded_file:
                return downloaded_file, os.path.basename(downloaded_file), "audio/mpeg"
            else:
                raise Exception("Le fichier audio n'a pas pu être trouvé après le téléchargement.")
...
# Titre principal avec glitch avancé
if not st.session_state.title_typed:
    ...
    # L'attribut data-text est nécessaire pour l'effet glitch
